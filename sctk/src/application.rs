...
use iced_futures::{
    ...
    subscription, Executor, Runtime, Subscription,
};
use tracing::error;
...
use iced_runtime::{
    ...
    system, user_interface, Action, Debug, Task, UserInterface,
};
...
pub enum Event<Message> {
    ...
    LayerSurface(platform_specific::wayland::layer_surface::Action),
    /// window requests from the client
    Window(platform_specific::wayland::window::Action),
    /// popup requests from the client
    Popup(platform_specific::wayland::popup::Action),
    /// data device requests from the client
    DataDevice(platform_specific::wayland::data_device::Action),
    /// xdg-activation request from the client
    Activation(platform_specific::wayland::activation::Action),
    ...
    A::Theme: DefaultStyle,
{
    ...
    let mut event_loop = SctkEventLoop::<Action<A::Message>>::new(&settings)
        .expect("Failed to initialize the event loop");

    let (mut runtime, ev_proxy) = {
    ...
    let task = match settings.surface {
        settings::InitialSurface::LayerSurface(b) => {
            Task::batch(vec![task, get_layer_surface(b)])
        }
        settings::InitialSurface::XdgWindow(b) => {
            Task::batch(vec![task, get_window(b)])
        }
        settings::InitialSurface::None => task,
    };

    if let Some(stream) = task.into_stream() {
        runtime.run(Box::pin(stream.map(|s| Event::Message(s))));
    }
    ...
    #[allow(unsafe_code)]
    let compositor = C::new(graphics_settings, wrapper.clone()).await.unwrap();
    ...
    let (mut sender, receiver) =
        mpsc::unbounded::<IcedSctkEvent<Action<A::Message>>>();
...
async fn run_instance<A, E, C>(
    ...
    mut runtime: Runtime<
        E,
        proxy::Proxy<Event<Action<A::Message>>>,
        Event<Action<A::Message>>,
    >,
    mut ev_proxy: proxy::Proxy<Event<Action<A::Message>>>,
    mut debug: Debug,
    mut receiver: mpsc::UnboundedReceiver<IcedSctkEvent<Action<A::Message>>>,
    ...
    queue_handle: QueueHandle<SctkState<Action<<A as Program>::Message>>>,
    ...
    A::Theme: DefaultStyle,
{
    ...
    let mut subsurface_state = None::<SubsurfaceState<Action<A::Message>>>;

    let subscription = application
        .subscription()
        .map(|s| Event::Message(Action::Output(s)));
    ...
    'main: while let Some(event) = receiver.next().await {
        match event {
            ...
            IcedSctkEvent::UserEvent(action) => {
                // TODO cleanup 
                // should this just be re-written when we make a shell for combined sctk/winit?
                // handle_actions(
                //     &application,
                //     &mut cache,
                //     state,
                ...
                while i < sctk_events.len() {
                    ...
                    if remove {
                        ...
                        for native_event in event.to_native(
                            ...
                            ); // TODO(POP): Is this the right window id to use here?
                        }
                    } else {
                        i += 1;
                    }
...
pub fn build_user_interface<'a, A: Application>(
    ...
    auto_size_surfaces: &mut HashMap<
        ...
        (u32, u32, Limits, bool),
    >,
    ev_proxy: &mut proxy::Proxy<Event<Action<A::Message>>>,
        ...
        if dirty {
            match id {
                SurfaceIdWrapper::LayerSurface(inner) => {
                    ev_proxy.send_event(
                        Event::LayerSurface(
                            platform_specific::wayland::layer_surface::Action::Size { id: inner, width: Some(w), height: Some(h) },
                        )
                    );
                }
...
pub(crate) fn update<A, E, C>(
    ...
    proxy: &mut proxy::Proxy<Event<Action<A::Message>>>,
    debug: &mut Debug,
    messages: &mut Vec<A::Message>,
    actions: &mut Vec<Action<A::Message>>,
...
type MyRuntime<'a, E, M> =
    &'a mut Runtime<E, proxy::Proxy<Event<Action<M>>>, Event<Action<M>>>;
    ...
    C: Compositor<Renderer = A::Renderer> + 'static,
{
    match action {
            Action::Clipboard(action) => match action {
                ...
                clipboard::Action::ReadData(allowed, channel, kind) => {
            ...
            Action::Window(action)  => {
                if let Ok(a) = action.try_into() {
                    return handle_actions(application, cache, state, renderer, Action::PlatformSpecific(platform_specific::Action::Wayland(platform_specific::wayland::Action::Window(a))), runtime, proxy, debug, _graphics_info, auto_size_surfaces, clipboard);
                }
            }
            Action::Window(action) => {}
            ...
            Action::Widget(action) => {
                ...
                *cache = current_cache;
                ...
                // TODO cleanup
                // .and_then(|o| match *o {
                //     OperationWrapper::Message(o) => Some(Action::Widget(o)),
                //     _ => None
                // });
            }
            Action::PlatformSpecific(
                ...
                if let platform_specific::wayland::layer_surface::Action::LayerSurface{ mut builder} = layer_surface_action {
                    ...
                    proxy.send_event(Event::LayerSurface(platform_specific::wayland::layer_surface::Action::LayerSurface {builder}));
                } else {
            ...
            Action::PlatformSpecific(
                ...
                if let platform_specific::wayland::window::Action::Window{ mut builder} = window_action {
                    if builder.autosize {
                    ...
                    proxy.send_event(Event::Window(platform_specific::wayland::window::Action::Window{builder}));
                } else {
            ...
            Action::PlatformSpecific(
                ...
                if let popup::Action::Popup { mut popup } = popup_action {
                    ...
                    proxy.send_event(Event::Popup(popup::Action::Popup{popup}));
                } else {
                    proxy.send_event(Event::Popup(popup_action));
                }
            }
            Action::PlatformSpecific(platform_specific::Action::Wayland(platform_specific::wayland::Action::DataDevice(data_device_action))) => {
            ...
            Action::PlatformSpecific(platform_specific::Action::Wayland(platform_specific::wayland::Action::SessionLock(session_lock_action))) => {
